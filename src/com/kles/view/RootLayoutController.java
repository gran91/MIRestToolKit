...
public class RootLayoutController {
    ...
    public void setMainApp(MainApp mainApp) {
        this.mainApp = (MIRestTools) mainApp;
        this.mainApp.getListSkin().entrySet().stream().forEach((nameSkin) -> {
            ...
            menuItemSkin.setOnAction(e -> {
                ...
                this.mainApp.prefs.put(MIRestTools.SKIN, nameSkin.getKey());
        ...
        langmenu.getItems().stream().forEach(m -> {
            ((RadioMenuItem) m).setSelected(this.mainApp.prefs.get(MIRestTools.LANGUAGE, Locale.getDefault().getLanguage()).equals(m.getId()));
            m.setOnAction(e -> {
                Locale.setDefault(new Locale(m.getId()));
                this.mainApp.prefs.put(MIRestTools.LANGUAGE, m.getId());
                langmenu.getItems().stream().forEach(m1 -> {
                    ((RadioMenuItem) m).setSelected(m1.getId().equals(this.mainApp.prefs.get(MIRestTools.LANGUAGE, Locale.getDefault().getLanguage())));
                });
                try {
                ...
                new MIRestTools().start(new Stage());
            });
        });
    }
    ...
    public void showMDBREAD() {
        try {
            FXMLLoader loader = new FXMLLoader();
            loader.setResources(mainApp.getResourceBundle());
            loader.setLocation(MIRestTools.class.getResource("/com/kles/view/mi/MDBREADTools.fxml"));
            Pane miRestTest = loader.load();
